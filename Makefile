...
# Distributable release: zipped and tarred. Binaries are laid out flat so the engine
# resolves beside the launcher (or gets setcap'd by install.sh on a non-nosuid prefix).
VERSION ?= 0.1.0
DIST_NAME := openflux-$(VERSION)-$(shell uname -m)
GUI ?= 0
dist: cargo engine
	...
	@cd dist && tar czf $(DIST_NAME).tar.gz $(DIST_NAME) && (cd $(DIST_NAME) && zip -qr ../$(DIST_NAME).zip .)
	@echo "dist ready: dist/$(DIST_NAME).tar.gz, dist/$(DIST_NAME).zip"
